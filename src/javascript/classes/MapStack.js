...
export default class MapStack{
    ...
    merge(easyMap){
        for (let [key,value] of easyMap.entries()) {
    ...
    popItems(){
        for (let [key,value] of this.map.entries()) { //Since it's a generator, it's not performance killer
            let anItemOfAnyX = value;
            this.map.delete(key);
            return anItemOfAnyX;
        }
