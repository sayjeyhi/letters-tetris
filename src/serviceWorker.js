...
Cache polyfill
 */
!(function() { let t=Cache.prototype.addAll, e=navigator.userAgent.match(/(Firefox|Chrome)\/(\d+\.)/); if (e) var r=e[1], n=parseInt(e[2]); t&&(!e||r==='Firefox'&&n>=46||r==='Chrome'&&n>=50)||(Cache.prototype.addAll=function(r) { const n=this; function o(t) { this.name='NetworkError', this.code=19, this.message=t; } return o.prototype=Object.create(Error.prototype), Promise.resolve().then(function() { if (arguments.length<1) throw new TypeError(); return r=r.map(t => { return t instanceof Request?t:String(t); }), Promise.all(r.map(t => { typeof t==='string'&&(t=new Request(t)); const e=new URL(t.url).protocol; if (e!=='http:'&&e!=='https:') throw new o('Invalid scheme'); return fetch(t.clone()); })); }).then(t => { if (t.some(t => { return !t.ok; })) throw new o('Incorrect response status'); return Promise.all(t.map((t, e) => { return n.put(r[e], t); })); }).then(() => {}); }, Cache.prototype.add=function(t) { return this.addAll([t]); }); }());
